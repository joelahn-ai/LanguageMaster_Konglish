...
import {summarizeContent,scopeNote} from './catalog.js';
export function createViews(env){
  ...
  function home(){
    ...
    app.innerHTML=`<div class="intro"><div><div class="eyebrow">나의 영어, 한 단어씩</div><h1>작은 단어 하나로<br>일상의 말을 넓혀 보세요.</h1><p>듣고, 읽고, 내 문장으로 남기는 나만의 단어집.</p></div><span class="sample-note">${wordGroups(items).length}개 단어 · ${items.length}개 뜻</span></div>${env.scopeControl()}<div class="collections">${editions.map((e,i)=>{
      ...
      if(!items.some(w=>w.level===i))return `<section class="collection" style="--accent:${e.color}"><div class="eyebrow">0${i+1} / ${e.label} 단어집</div><div class="number">—</div><div class="muted">이번 버전에 미포함</div><h2>${e.title}</h2><p class="description">이 버전은 기본·중급 단어를 먼저 학습할 수 있어요.</p><button data-start="${i}" disabled>이번 버전에 미포함</button></section>`;
    ...
    const note=scopeNote(env.catalog.find(s=>s.id===env.context().version)?.releaseScope);
    if(note)app.querySelector('.intro').insertAdjacentHTML('afterend',`<p class="hint">${esc(note)}</p>`);
