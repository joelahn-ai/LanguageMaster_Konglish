...
function renderLearn(){
  if(!activeId&&!pool().length){app.innerHTML='<section class="empty"><h1>이 범위에는 아직 단어가 없어요.</h1><p>단어집 화면에서 기본이나 중급을 선택해 주세요. 기존 문장과 진도는 보존되어 있어요.</p><a class="text-link" href="#home">단어집 선택 →</a></section>';return;}
  ...
  <article class="study-card" aria-label="${esc(w.text)} 학습 카드"><div class="top-navigation"><button id="previous" ${trailIndex>0?'':'disabled'}>← 이전</button><button class="primary" id="next" aria-label="다음 단어">다음 →</button></div><div class="card-top"><span>${e.label} · ${esc(w.category)}</span><button class="favorite" id="favorite" aria-pressed="${record.favorite}">${record.favorite?'★ 즐겨찾는 단어':'☆ 즐겨찾기'}</button></div><div class="word-heading"><h2 class="word" lang="en">${esc(w.text)}${forms.length>1?'<a class="form-star" href="#word-forms" aria-label="다른 품사와 뜻 보기">*</a>':''}</h2><button class="listen" id="listen" aria-label="${esc(w.text)} 발음 듣기" title="미국식 발음 듣기">${speaker}</button></div>${forms.length>1?`<div id="word-forms" class="toolbar" role="group" aria-label="품사와 뜻 선택">${forms.map((f,i)=>`<button data-form="${esc(f.id)}" aria-pressed="${f.id===w.id}">${esc(f.word.pos)}${forms.filter(other=>other.word.pos===f.word.pos).length>1?` · ${esc(f.meaning)}`:''}${i===0?' · 기본':''}</button>`).join('')}</div>`:''}<div class="phonetic">미국식 <span lang="en">${esc(w.pronunciation.ipa)}</span> · ${esc(w.word.pos)}</div><p class="meaning">${esc(w.meaning)}</p><div class="hint" id="voice-note"></div>
  ...
  <div class="card-actions"><button id="make-download">이미지 저장</button></div><p class="draft-note">${reviewLabel(w)}. 작성한 문장은 이 기기에만 저장돼요.</p><details class="draft-note"><summary>콘텐츠 검토 정보</summary><p>${esc(w.review.note)}</p></details></article></div>`;
