...
import {studyPool,emptyState,loadState,saveState,mergeBackup,validateState,pickNext,entryFor,touchEntry,STORAGE_KEY} from './state.js';
...
function renderSaved(){
  ...
  app.innerHTML=`<div class="page-heading"><div class="eyebrow">나의 말이 된 단어들</div><h1>내 카드</h1><p class="muted">즐겨찾는 단어와 직접 쓴 문장을 다시 만나 보세요.</p></div><div class="toolbar" aria-label="내 카드 필터">${[['all','전체'],['favorites','즐겨찾기'],['written','문장 있는 카드']].map(([id,label])=>`<button data-filter="${id}" aria-pressed="${filter===id}">${label}</button>`).join('')}</div><p class="muted">${saved.length}개의 카드</p><div class="saved-list">${saved.map(w=>{const r=entryFor(state,w.id);return `<article class="saved-item"><div><h2 lang="en">${esc(w.text)}${r.favorite?' <span aria-label="즐겨찾기">☆</span>':''}</h2><p>${esc(w.meaning)}</p>${r.sentence?`<p class="saved-sentence" lang="en">${esc(r.sentence)}</p>`:''}</div><button data-open="${esc(w.id)}">문장 수정 · 공유</button></article>`;}).join('') || '<div class="empty">아직 모아 둔 카드가 없어요.<br>단어에 별을 누르거나 내 문장을 써 보세요.<br><a class="text-link" href="#learn">단어 만나러 가기 →</a></div>'}</div>`;
  app.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{filter=b.dataset.filter;renderSaved();showStorageWarning();});
  app.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>{const w=items.find(i=>i.id===b.dataset.open);state.settings.level=state.settings.studyMode==='stage'?w.level:Math.max(level(),w.level);persist();study(w.id);});
}
function renderSettings(){
  ...
  <section class="settings-panel"><h2>콘텐츠와 오프라인</h2><p id="offline-status">${esc(offlineStatus)}</p><p>단어 ${items.length}개 · 예문 ${items.reduce((n,w)=>n+w.examples.length,0)}개<br>버전 ${esc(CONTENT_VERSION)}<br>저장 완료 후 단어 보기, 문장 작성, 카드 이미지 만들기를 오프라인으로 이용할 수 있어요.</p><p>기기 저장 공간 정리로 앱 데이터가 삭제될 수 있어요. 중요한 문장은 백업해 주세요.</p></section>
  ...
  bindScope();$('#choose-backup').onclick=()=>{$('#restore').value='';$('#restore-preview').replaceChildren();$('#restore').click();};
